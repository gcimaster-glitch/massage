...
app.post('/api/admin/run-migration', async (c) => {
  ...
  if (action === 'rebuild_bookings') {
    ...
    statements = [
      ...
      `DROP TABLE IF EXISTS reviews`,
      `DROP TABLE IF EXISTS therapist_menu_courses`,
      `DROP TABLE IF EXISTS therapist_menu_options`,
      `DROP TABLE IF EXISTS therapist_schedules`,
      `DROP TABLE IF EXISTS therapist_profiles`,
      `DROP TABLE IF EXISTS sites`,
      `DROP TABLE IF EXISTS users`,
      // テーブル再作成
      `CREATE TABLE users (id TEXT PRIMARY KEY, email TEXT UNIQUE NOT NULL, name TEXT NOT NULL, phone TEXT, role TEXT NOT NULL CHECK (role IN ('USER','THERAPIST','HOST','THERAPIST_OFFICE','AFFILIATE','ADMIN')), avatar_url TEXT, email_verified INTEGER DEFAULT 0, phone_verified INTEGER DEFAULT 0, kyc_status TEXT DEFAULT 'PENDING' CHECK (kyc_status IN ('PENDING','VERIFIED','REJECTED')), created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP, is_archived INTEGER DEFAULT 0, archived_at DATETIME, password_hash TEXT, login_failed_count INTEGER DEFAULT 0, locked_until INTEGER DEFAULT NULL, last_login_at INTEGER DEFAULT NULL, last_login_ip TEXT DEFAULT NULL, stripe_customer_id TEXT)`,
      `CREATE TABLE therapist_profiles (id TEXT PRIMARY KEY, user_id TEXT UNIQUE NOT NULL, office_id TEXT, bio TEXT, specialties TEXT, experience_years INTEGER, certifications TEXT, rating REAL DEFAULT 0, review_count INTEGER DEFAULT 0, approved_areas TEXT, status TEXT DEFAULT 'PENDING' CHECK (status IN ('PENDING','APPROVED','SUSPENDED','REJECTED')), created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP, catch_copy TEXT, radar_data TEXT, qualification_uploads TEXT, bank_name TEXT, bank_branch TEXT, bank_branch_code TEXT, bank_account_type TEXT, bank_account_number TEXT, bank_account_holder TEXT, outcall_available INTEGER DEFAULT 0, incall_available INTEGER DEFAULT 0, base_location TEXT, base_lat REAL, base_lng REAL, travel_methods TEXT, outcall_hours TEXT, incall_hours TEXT, FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE)`,
      `CREATE TABLE therapist_schedules (id TEXT PRIMARY KEY, therapist_id TEXT NOT NULL, date_val TEXT, day_of_week INTEGER, start_time TEXT NOT NULL, end_time TEXT NOT NULL, is_available INTEGER DEFAULT 1, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (therapist_id) REFERENCES users(id) ON DELETE CASCADE)`,
      `CREATE TABLE therapist_menu_courses (id TEXT PRIMARY KEY, therapist_profile_id TEXT NOT NULL, name TEXT NOT NULL, duration INTEGER NOT NULL, price INTEGER NOT NULL, description TEXT, display_order INTEGER DEFAULT 0, is_active INTEGER DEFAULT 1, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (therapist_profile_id) REFERENCES therapist_profiles(id) ON DELETE CASCADE)`,
      `CREATE TABLE therapist_menu_options (id TEXT PRIMARY KEY, therapist_profile_id TEXT NOT NULL, name TEXT NOT NULL, price INTEGER NOT NULL, description TEXT, display_order INTEGER DEFAULT 0, is_active INTEGER DEFAULT 1, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (therapist_profile_id) REFERENCES therapist_profiles(id) ON DELETE CASCADE)`,
      `CREATE TABLE sites (id TEXT PRIMARY KEY, host_id TEXT NOT NULL, name TEXT NOT NULL, type TEXT NOT NULL CHECK (type IN ('HOTEL','OFFICE','HOME','OTHER','CARE_CUBE','PRIVATE_SPACE','CHARGE','PAUWAU')), address TEXT NOT NULL, area_code TEXT NOT NULL, latitude REAL, longitude REAL, room_count INTEGER DEFAULT 1, amenities TEXT, status TEXT DEFAULT 'PENDING' CHECK (status IN ('PENDING','APPROVED','SUSPENDED','REJECTED')), image_url TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (host_id) REFERENCES users(id) ON DELETE CASCADE)`,
      `CREATE TABLE bookings (id TEXT PRIMARY KEY, user_id TEXT, therapist_id TEXT NOT NULL, therapist_name TEXT, site_id TEXT, office_id TEXT, type TEXT NOT NULL CHECK(type IN ('ONSITE','MOBILE','HOTEL','HOME','OFFICE','OUTCALL')), status TEXT DEFAULT 'PENDING' CHECK(status IN ('PENDING','CONFIRMED','IN_PROGRESS','COMPLETED','CANCELLED')), service_name TEXT NOT NULL DEFAULT '施術', duration INTEGER NOT NULL, scheduled_at DATETIME, scheduled_start DATETIME, scheduled_end DATETIME, price INTEGER NOT NULL, payment_intent_id TEXT, payment_status TEXT DEFAULT 'PENDING' CHECK(payment_status IN ('PENDING','PAID','REFUNDED')), notes TEXT, guest_name TEXT, guest_email TEXT, guest_phone TEXT, timelock_id TEXT, customer_address TEXT, postal_code TEXT, customer_lat REAL, customer_lng REAL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, completed_at DATETIME, FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE SET NULL, FOREIGN KEY (therapist_id) REFERENCES users(id) ON DELETE CASCADE)`,
      `CREATE TABLE booking_items (id TEXT PRIMARY KEY, booking_id TEXT NOT NULL, item_type TEXT, item_id TEXT, item_name TEXT, price INTEGER, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (booking_id) REFERENCES bookings(id) ON DELETE CASCADE)`,
      `CREATE TABLE booking_timelocks (id TEXT PRIMARY KEY, therapist_id TEXT NOT NULL, date TEXT NOT NULL, start_time TEXT NOT NULL, end_time TEXT NOT NULL, booking_id TEXT, status TEXT DEFAULT 'LOCKED', created_at DATETIME DEFAULT CURRENT_TIMESTAMP, expires_at DATETIME)`,
      `CREATE TABLE reviews (id TEXT PRIMARY KEY, booking_id TEXT NOT NULL, therapist_id TEXT NOT NULL, user_id TEXT, rating INTEGER NOT NULL CHECK(rating BETWEEN 1 AND 5), comment TEXT, customer_age_range TEXT, customer_gender TEXT, customer_occupation TEXT, body_concerns TEXT DEFAULT '[]', ng_items TEXT DEFAULT '[]', is_public INTEGER NOT NULL DEFAULT 1, therapist_reply TEXT, therapist_replied_at INTEGER, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL)`,

  ...
  try {
    if (!c.env.DB) {
      ...
      const mockUser = {
        id: 'admin-001',
        email: email,
        name: email === 'admin@hogusy.com' ? '総管理者' : '山田 太郎',
        role: email === 'admin@hogusy.com' ? 'ADMIN' : 'USER'
      }
      
      // JWT形式のトークンを生成（header.payload.signature）
    ...
    // パスワード検証（簡易版 - 本番環境では bcrypt などを使用）
    // デモ用: password === 'demo123' または password_hash が null なら通す
    if (password !== 'demo123' && user.password_hash && user.password_hash !== 'default') {
      return c.json({ error: 'Invalid credentials' }, 401)
    }

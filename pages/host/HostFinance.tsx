      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">拠点収益の明細と支払明細書</p>
          </div>
        </div>
            ...
            {activeTab === 'overview' && (
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-5">
                  <h3 className="font-semibold text-gray-900 mb-2">ホスト収益の仕組み</h3>
                  <p className="text-sm text-gray-600">
                    お客様がお支払いいただいた施術料金の <span className="font-bold text-teal-600">5%</span> がホスト収益として自動計算されます。
                    施術完了後、HOGUSYシステムが自動的に収益明細に記録し、毎月末に支払明細書が発行されます。
                  </p>
                </div>
                      ...
                      <div key={split.id} className="px-5 py-3 flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-gray-400">
                            {split.site_name && `${split.site_name} / `}
                        ...
                        <div className="text-right">
                          <p className="text-sm font-bold text-gray-900">{formatCurrency(split.net_amount)}</p>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="text-left px-4 py-3 text-xs font-medium text-gray-500">拠点</th>
                        <th className="text-right px-4 py-3 text-xs font-medium text-gray-500">施術料金</th>
                        <th className="text-right px-4 py-3 text-xs font-medium text-gray-500">収益率</th>
                        <th className="text-right px-4 py-3 text-xs font-medium text-gray-500">収益額</th>
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {splits.map((split) => (
                        <tr key={split.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 text-gray-600">{split.site_name || '—'}</td>
                          ...
                          <td className="px-4 py-3 text-right font-semibold text-gray-900">{formatCurrency(split.net_amount)}</td>
                      ...
                      {splits.length === 0 && (
                        <tr>
                          <td colSpan={7} className="px-4 py-10 text-center text-gray-400">収益明細はまだありません</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
                    ...
                    {statements.map((stmt) => (
                      <div key={stmt.id} className="px-5 py-4 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          {stmt.pdf_url && (
                            <a href={stmt.pdf_url} target="_blank" rel="noopener noreferrer" className="text-sm text-teal-600 hover:underline">
                              PDF
                            </a>
                          )}
                        </div>

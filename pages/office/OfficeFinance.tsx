...
interface EarningSplit {
  ...
  gross_amount: number
  rate: number
  amount: number // transaction_splitsのamount（net_amount相当）
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">収益・財務管理</h1>
            <p className="text-sm text-gray-500 mt-1">オフィスの収益明細と支払明細書</p>
          </div>
        </div>
            ...
            {activeTab === 'overview' && (
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-5">
                  <h3 className="font-semibold text-gray-900 mb-3">収益分配の仕組み</h3>
                  <p className="text-sm text-gray-600 mb-4">
                    お客様がお支払いいただいた施術料金は、以下の割合でHOGUSYシステムが自動分配します。
                  </p>
                  <div className="flex flex-wrap gap-6">
                    {[
                      { label: 'セラピスト', rate: '70%', color: 'bg-green-500' },
                      { label: 'オフィス（貴社）', rate: '10%', color: 'bg-blue-500' },
                      { label: 'ホスト', rate: '5%', color: 'bg-purple-500' },
                      { label: 'HOGUSY本部', rate: '10%', color: 'bg-indigo-500' },
                      { label: '販促費', rate: '5%', color: 'bg-orange-500' },
                    ].map((item) => (
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="text-right px-4 py-3 text-xs font-medium text-gray-500">分配率</th>
                        ...
                        <div className="flex items-center gap-4">
                          <div className="text-right">
                            <p className="font-bold text-gray-900">
                              {formatCurrency(stmt.net_amount)}
                            </p>
                            <p className="text-xs text-gray-400">
                              総額 {formatCurrency(stmt.total_amount)}
                              {stmt.deduction_amount > 0 &&
                                ` − 控除 ${formatCurrency(stmt.deduction_amount)}`}
                            </p>
                          </div>
